...
Comparative study of six pathfinding algorithms on procedurally generated mazes: DFS, BFS, A\* (Manhattan), A\* (Corridor-Aware), MDP Value Iteration, and MDP Policy Iteration.
...
#    Select an algorithm from the dropdown → click Play
```

### Running the benchmark

```bash
# Full automated benchmark (Selenium, ~2 hours)
python benchmark_browser.py

# Results are written to benchmark_results.csv (or benchmark_results_g.csv)
```

### Generating outputs

```bash
# Plots (16 figures → plots/)
python visualize_results.py

# LaTeX metric tables (→ tables/)
python generate_metric_tables.py
```
...
```
path_finding/
├── path_finding/               # Algorithm implementations (installed as a package)
...
│       └── backend/server.py   # Flask API bridging frontend ↔ path_finding package
│
├── benchmark_browser.py        # Selenium-driven benchmark harness
├── benchmark_results_g.csv     # Raw benchmark data (94 runs × 6 algorithms)
...
Mazes are procedurally generated at six sizes (7×7 to 75×75) with varying path counts (1–20 alternative routes). Start and goal positions are randomised with a minimum Manhattan distance constraint. Each configuration runs multiple repetitions, producing 94 maze instances × 6 algorithms = 564 runs total.

### Metrics recorded

**Search/Planning:**
- Execution time, nodes visited, search coverage, search efficiency ratio, time per node

**Path Quality:**
- Path length, turn count, optimality ratio (vs BFS baseline), suboptimality percentage

**Memory:**
- Peak frontier size, peak memory usage

**MDP-specific:**
- Planning iterations, planning time per state, extraction time per state, cumulative reward, discounted return
...
| `14_success_and_coverage_by_paths.png` | Effect of path count on performance |
| `15_memory_metrics.png` | Memory usage comparison |
| `suboptimality_breakdown.png` | DFS suboptimality breakdown |

### Tables (`tables/`)

- Per-algorithm metric tables (`{Algorithm}_metrics_table.tex`)
- Consolidated comparison table (`consolidated_metrics_table.tex`)
- MDP planning metrics (`mdp_planning_metrics_table.tex`)
- MDP reward by maze size (`mdp_reward_by_size_table.tex`)
